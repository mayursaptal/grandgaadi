...
use WPCFE_HTML5_Tokenizer;
use WPCFE_HTML5_TreeBuilder;
...
class Dompdf
{
    ...
    */
     ...
     * Class constructor
     *
     * @param array|WPCFE_Options $options
     */
    public function __construct($options = null)
    {
        ...
        if (isset($options) && $options instanceof WPCFE_Options) {
            $this->setOptions($options);
        } elseif (is_array($options)) {
            $this->setOptions(new WPCFE_Options($options));
        } else {
            $this->setOptions(new WPCFE_Options());
        }
    ...
    public function loadHtmlFile($file, $encoding = null)
    {
        ...
        if ($protocol == "" || $protocol === "file://") {
            ...
            if ($chroot && strpos($realfile, $chroot) !== 0) {
                throw new Exception("Permission denied on $file. The file could not be found under the directory specified by WPCFE_Options::chroot.");
            }
    ...
    public function loadHtml($str, $encoding = null)
    {
        ...
        try {
            ...
            if ($this->options->isHtml5ParserEnabled() && class_exists(WPCFE_HTML5_Tokenizer::class)) {
                $tokenizer = new WPCFE_HTML5_Tokenizer($str);
                $tokenizer->parse();
                $doc = $tokenizer->save();
                ...
                $quirksmode = ($tokenizer->getTree()->getQuirksMode() > WPCFE_HTML5_TreeBuilder::NO_QUIRKS);
            } else {
    ...
    /**
     * @param WPCFE_Options $options
     * @return $this
     */
    public function setOptions(WPCFE_Options $options)
    {
